    ...
    <div className="min-h-dvh bg-background flex flex-col relative overflow-hidden">
            ...
            {step === 0 && (
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-4 rounded-2xl bg-secondary border border-border text-foreground text-lg placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-neon-blue/50 transition-all"
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full px-4 py-4 rounded-2xl bg-secondary border border-border text-foreground text-lg placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-neon-blue/50 transition-all pr-12"
                    placeholder="City or area"
                    autoFocus
                  />
                  <button
                    onClick={() => setLocation('Downtown, NYC')}
                    className="absolute right-3 top-1/2 -translate-y-1/2 p-2 text-neon-blue"
                  >
                ...
                <button
                  onClick={() => setLocation('Downtown, NYC')}
                  className="mt-3 flex items-center gap-2 text-sm text-neon-blue"
                >
              ...
              <div>
                ...
                <div className="flex flex-col items-center gap-8">
                  <div className="text-center">
                    <motion.div
                      key={getFitnessLabel(fitnessLevel)}
                      initial={{ scale: 0.8, opacity: 0 }}
                      animate={{ scale: 1, opacity: 1 }}
                      className={`text-4xl font-bold ${getFitnessColor(fitnessLevel)}`}
                    >
                  ...
                  <div className="w-full">
                    <input
                      ...
                      className="w-full h-2 rounded-full appearance-none bg-secondary cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-6 [&::-webkit-slider-thumb]:h-6 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-neon-blue [&::-webkit-slider-thumb]:shadow-[0_0_12px_hsl(187,100%,50%,0.5)]"
                    />
                  ...
                  {SPORTS.map(sport => {
                    ...
                    return (
                      <motion.button
                        ...
                        className={`px-4 py-3 rounded-2xl text-sm font-medium flex items-center gap-2 transition-all ${
                          isSelected
                      ...
                      <div key={sport} className="p-4 rounded-2xl bg-secondary border border-border">
                        ...
                        <div className="flex gap-2">
                          {(['beginner', 'intermediate', 'advanced'] as const).map(level => (
                            <button
                              key={level}
                              onClick={() => setExperience(sport, level)}
      ...
      <div className="fixed bottom-0 left-0 right-0 p-6 z-10">
        <motion.button
          ...
          disabled={!canProceed()}
          onClick={handleNext}
          className="w-full max-w-sm mx-auto flex items-center justify-center gap-2 py-4 rounded-2xl bg-neon-blue text-primary-foreground font-semibold text-base neon-glow-blue disabled:opacity-30 disabled:shadow-none transition-all"
        >
          {step === totalSteps - 1 ? (
            <>
