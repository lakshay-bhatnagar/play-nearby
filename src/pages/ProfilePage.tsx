...
import { Settings, ChevronRight, MapPin, Award, LogOut, Edit3, Save, X } from 'lucide-react';
  ...
  return (
    <div className="min-h-dvh bg-background pb-28">
      ...
      <div className="px-5 pt-14 pb-2 relative z-10 flex items-center justify-between">
        ...
        <button onClick={() => navigate('/settings')} className="p-2 text-muted-foreground active:scale-90 transition-transform">
      ...
      <div className="px-5 relative z-10">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-4 mb-8 mt-4">
            ...
            {editing ? (
              ...
              <>
                ...
                <div className="flex items-center gap-1.5 text-sm text-muted-foreground mt-0.5">
                  <MapPin className="w-3.5 h-3.5" />{profile?.location || 'No location set'}
                </div>
              </>
            )}
          </div>
          {editing ? (
            ...
            <button onClick={startEdit} className="p-2.5 rounded-2xl bg-secondary border border-border active:scale-90 transition-transform">
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="mb-6">
            ...
            <div className="flex flex-col gap-2">
              {sports.map(sport => {
                ...
                const level = exp?.level || 'beginner';
                return (
                  <div key={sport} className="p-4 rounded-2xl bg-card border border-border flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-medium text-foreground text-sm">{sport}</span>
                    </div>
                    <span className={`text-xs font-medium px-3 py-1 rounded-full capitalize ${level === 'advanced' ? 'bg-neon-orange/15 text-neon-orange' : level === 'intermediate' ? 'bg-neon-blue/15 text-neon-blue' : 'bg-neon-green/15 text-neon-green'}`}>{level}</span>
                  </div>
                );
              })}
            </div>
        ...
        <div className="flex flex-col gap-2">
          {[
            ...
            { label: 'Booking History', icon: Award, path: '/booking-history' },
          ].map(item => (
